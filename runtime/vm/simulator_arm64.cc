...
void Simulator::DecodeLoadStoreRegPair(Instr* instr) {
  ...
  if (instr->Bit(26) == 1) {
    ...
    if (instr->Bit(22)) {
      ...
      const bool signed = instr->Bit(30) == 1;
...
static int64_t divide64(int64_t top, int64_t bottom, bool signed) {
...
static int32_t divide32(int32_t top, int32_t bottom, bool signed) {
  // ARM64 does not trap on integer division by zero. The destination register
  // is instead set to 0.
  if (bottom == 0) {
    return 0;
...
void Simulator::DecodeMiscDP2Source(Instr* instr) {
  ...
  switch (op) {
    ...
    case 3: {
      ...
      const bool signed = instr->Bit(10) == 1;
      if (instr->SFField() == 1) {
        set_register(instr, rd, divide64(rn_val64, rm_val64, signed), R31IsZR);
      } else {
        set_wregister(rd, divide32(rn_val32, rm_val32, signed), R31IsZR);
      }
      break;
    }
    case 8: {
